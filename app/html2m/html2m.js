/**
 * HTML2M
 * TODO: Review code structure and params validation.
 */
...
module.exports = $wt => {
  ...
  $wt.route.form("/webtools/rest/html2m/convert", {
    cors: true,
    toto: "tutu"
  }, async (req, res) => {
     ...
     */
      ...
      urlAddress = process.env.HTTP_FULLPATH + "/webtools/rest/html2m/output/" + inputName;
    ...
    if (process.env.PROXY === 1) {
      browserArgs.push("--proxy-server=" + process.env.PROXY_HOST);
    }
    ...
    let outputFileName = "html2m_" + $wt.id() + "." + outputType;
    ...
    res.send({
      wtstatus: {
        ...
        output: process.env.HTTP_FULLPATH + "/webtools/rest/html2m/output/" + outputFileName
      }
    })
  ...
  $wt.route.file("/webtools/rest/html2m/output/*", {cors: true}, (req, res) => {

    let file = req.params[0];
    let path = config.folder + '/' + file;
  ...
  $wt.route.static("/webtools/demo/html2m", __dirname + "/demo");
  ...
  $wt.on('ready', async () => {
    ...
    try {
      ...
      console.log("\033[31m HTML2M:\033[0m Your html2m folder was not created.");
      process.exit();
    }
    ...
    console.log("\033[32m DEMO - HTML2M:\033[0m \t", process.env.HTTP_FULLPATH + "/webtools/demo/html2m");
