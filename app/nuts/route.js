...
module.exports = async $wt => {
  ...
  let nuts = await $wt.db.mysql('nuts').query("SELECT DISTINCT(cntr_id), year FROM geojson WHERE level=0").catch(() => {
    return [];
  });
  ...
  $wt.route.static("/webtools/demo/nuts", __dirname + "/demo");

  /**
   * REQUEST GEOJSON FILE
   */

  $wt.route.rest("/webtools/rest/nuts*", { }, async (req, res, next) => {
    ...
    let request = await $wt.db.mysql('nuts').query(sql, [params.scale, params.year, params.level]).catch(() => {
      return [];
    });
  ...
  $wt.on('ready', async () => {
    ...
    // Drop a message.
    console.log("\033[32m DEMO - NUTS:\033[0m \t\t", process.env.HTTP_FULLPATH + "/webtools/demo/nuts");
